...
class MohsinSchoolCourseBatch(models.Model):
    _name = 'mohsin.school.course.batch'
    _description = 'Course Batch'


    def _default_year_id(self):
        # Search for the active academic year
        active_year = self.env['mohsin.school.year'].search([('active', '=', True)], limit=1)
    ...
    subject_ids = fields.Many2many('mohsin.school.subject', compute='_compute_subjects')
    company_id = fields.Many2one('res.company', string='Company', index=True, default=lambda self: self.env.company)
    year_id = fields.Many2one('mohsin.school.year', string='Year', required=True,
    ...
    def _compute_subjects(self):
        ...
        filtered_subject_lines = self.env['mohsin.school.course.subject.line'].browse([
