...
class CourseGradingType(models.Model):
    _name = 'mohsin.school.course.grading.type'
    _description = 'Mohsin Course Grading Type'
    name = fields.Char(string='Type', required=True, index=True)


class MohsinSchoolCourse(models.Model):
    _name = 'mohsin.school.course'
    _description = 'Mohsin School Course'

    name = fields.Char(string='Course', required=True, index=True)
    code = fields.Char(string='Code', required=True)
    complete_name = fields.Char('Complete Name', compute='_compute_complete_name', recursive=True, store=True)
    parent_id = fields.Many2one('mohsin.school.course', string='Parent Course', index=True,
    ...
    sequence_id = fields.Many2one('ir.sequence', 'Roll Number Sequence', copy=False)
    batch_ids = fields.One2many('mohsin.school.course.batch', 'course_id', string="Batches")
    batch_count = fields.Integer(string='Batches', compute='_compute_course_batch_count')
    course_subject_line = fields.One2many('mohsin.school.course.subject.line', 'course_id', string="Subject Line")
    ...
    section_ids = fields.One2many('mohsin.school.course.section', 'course_id', string="Sections")
    ...
    def action_open_batch(self):
        action = self.env.ref('mohsin_school.action_course_batch').read()[0]
        action.update({
            'name': 'Batches',
            'view_mode': 'tree',
            'res_model': 'mohsin.school.course.batch',
    ...
    def action_open_section(self):
        action = self.env.ref('mohsin_school.action_school_seciton').read()[0]
        action.update({
            'name': 'Sections',
            'view_mode': 'tree',
            'res_model': 'mohsin.school.course.section',
            'type': 'ir.actions.act_window',
            'domain': [('course_id', '=', self.id)],
            'context': {
                'default_course_id': self.id,
...
class SchoolCourseSubjectLine(models.Model):
    _name = 'mohsin.school.course.subject.line'
    _description = 'Course Subject Line'

    name = fields.Char("Name")
    course_id = fields.Many2one('mohsin.school.course', string='Course', required=True, ondelete='cascade', index=True)
    company_id = fields.Many2one('res.company', related='course_id.company_id')
    subject_id = fields.Many2one('mohsin.school.subject', string='Subject', required=True)
    batch_ids = fields.Many2many(
        'mohsin.school.course.batch',
        'subject_batch_rel',
        string='Batches',
        column1='subject_id',
        column2='batch_id',
